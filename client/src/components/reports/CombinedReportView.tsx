...
type Props = { monthSummary: MonthSummary[]; rentMonthly: RentMonthly[]; from: string; to: string };

export function CombinedReportView({ monthSummary, rentMonthly, from, to }: Props) {
  const subtitle = from === to ? monthLabel(from) : `${monthLabel(from)} – ${monthLabel(to)}`;

  const months = [...new Set([...monthSummary.map(r => r.month), ...rentMonthly.map(r => r.month)])].sort();
  ...
  const totalIn = rentMonthly.reduce((s, r) => s + r.collected, 0);
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          { label: 'Rent Collected', value: totalIn, color: 'text-green-600 dark:text-green-400' },
          { label: 'Net Position', value: totalIn - totalOut, color: totalIn - totalOut >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' },
        ].map(c => (
      ...
      <table className="w-full text-sm border rounded-lg overflow-hidden">
        ...
        <tbody className="divide-y divide-border">
          {months.map(month => {
            const bills = monthSummary.find(r => r.month === month);
            const rent = rentMonthly.find(r => r.month === month);
            const net = (rent?.collected ?? 0) - (bills?.total ?? 0);
            return (
              <tr key={month}>
