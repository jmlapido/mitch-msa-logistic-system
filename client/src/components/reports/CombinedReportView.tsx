...
type Props = {
  ...
  writtenOffMonthly: WrittenOffMonthly[];
  from: string;
  to: string;
};

export function CombinedReportView({ monthSummary, rentMonthly, commissionsMonthly, writtenOffMonthly, from, to }: Props) {
  ...
  const months = [...new Set([
    ...
    ...writtenOffMonthly.map(r => r.month),
  ])].sort();
  ...
  const chartData = months.map(month => {
    ...
    const writtenOff = writtenOffMonthly.find(r => r.month === month);
    return {
      month: month.slice(5),
      Bills: bills?.total ?? 0,
      ...
      <table className="w-full text-sm border rounded-lg overflow-hidden">
        ...
        <tbody className="divide-y divide-border">
          {months.map(month => {
            ...
            const commissions = commissionsMonthly.find(r => r.month === month);
            const writtenOff = writtenOffMonthly.find(r => r.month === month);
            const net = (rent?.collected ?? 0) + (commissions?.total ?? 0) - (bills?.total ?? 0) - (writtenOff?.total ?? 0);
            return (
              <tr key={month}>
