...
type MonthSummary = { month: string; total: number; count: number; cash: number; cheque: number };
...
export function WithdrawalsReportView({ rows, monthSummary, from, to }: Props) {
  ...
  const grandCash = monthSummary.reduce((s, r) => s + (r.cash ?? 0), 0);
  const grandCheque = monthSummary.reduce((s, r) => s + (r.cheque ?? 0), 0);

  return (
    <div>
      <PrintHeader title="Withdrawals Report" subtitle={subtitle} />

      <div className="no-print mb-4">
        <h2 className="text-lg font-semibold">Withdrawals Report</h2>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6 max-w-2xl">
        ...
        <div className="bg-card border rounded-lg p-3 text-center">
          ...
          <p className="text-base font-bold">{<AedAmount amount={grandCash} />}</p>
        </div>
        <div className="bg-card border rounded-lg p-3 text-center">
          <p className="text-xs text-muted-foreground">Cheque</p>
          <p className="text-base font-bold">{<AedAmount amount={grandCheque} />}</p>
        </div>
        <div className="bg-card border rounded-lg p-3 text-center">
          ...
          <tbody className="divide-y divide-border">
            {monthSummary.map(r => (
              <tr key={r.month}>
                ...
                <td className="px-3 py-1.5 text-right text-muted-foreground">{<AedAmount amount={r.cash ?? 0} />}</td>
                <td className="px-3 py-1.5 text-right text-muted-foreground">{<AedAmount amount={r.cheque ?? 0} />}</td>
            ...
            <tr className="font-semibold bg-muted">
              ...
              <td className="px-3 py-1.5 text-right">{<AedAmount amount={grandCash} />}</td>
              <td className="px-3 py-1.5 text-right">{<AedAmount amount={grandCheque} />}</td>
      ...
      <div className="break-before-page">
        ...
        <table className="w-full text-xs border rounded-lg overflow-hidden">
          <thead className="bg-muted">
            <tr>
              <th className="text-left px-3 py-2">Withdrawn By</th>
          ...
          <tbody className="divide-y divide-border">
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-muted/20">
                <td className="px-3 py-1.5">{r.withdrawn_by}</td>
                <td className="px-3 py-1.5 text-right font-medium">{<AedAmount amount={r.amount} />}</td>
                <td className="px-3 py-1.5 text-center">{r.withdrawn_date}</td>
