...
type Props = {
  rows: Withdrawal[];
  onEdit: (row: Withdrawal) => void;
  canEdit: boolean;
};

export function WithdrawalsTable({ rows, onEdit, canEdit }: Props) {
  const { deleteWithdrawal } = useWithdrawalMutations();

  async function handleDelete(row: Withdrawal) {
    if (!confirm(`Delete this withdrawal of AED ${row.amount.toLocaleString()} by ${row.withdrawn_by}?`)) return;
    try {
      await deleteWithdrawal.mutateAsync(row.id);
  ...
  if (rows.length === 0) {
    return <p className="text-center py-12 text-muted-foreground">No withdrawals recorded for this month.</p>;
  }
    ...
    <table className="w-full text-sm border-collapse">
      <thead>
        <tr className="border-b text-left text-xs uppercase tracking-wider text-muted-foreground">
          <th className="py-2 pr-3">Withdrawn By</th>
      ...
      <tbody>
        {rows.map(row => (
          <tr key={row.id} className="border-b last:border-0">
            <td className="py-2 pr-3">{row.withdrawn_by}</td>
            <td className="py-2 pr-3"><AedAmount amount={row.amount} /></td>
            <td className="py-2 pr-3">{formatDate(row.withdrawn_date)}</td>
